...
//colocar a senha para o mysql caso seja necessário
var connection = mysql.createConnection({
	host     : 'localhost',
	user     : 'root',
	password : '',
	database : 'nodereactiftm'
});
...
app.get('/homepagetvmaze', function(request, response) {
	...
	if (username && password) {
		connection.query('SELECT * FROM usuarios WHERE username = ? AND password = ?', [username, password], function(error, results, fields) {
			if (results.length > 0) {
				...
				response.redirect('/homepagetvmaze')
